...
export default (tokens: Record<string, any>) => {
  return [
    ...
    scope(
      ["string.quoted.single", "string.template", "string.quoted.double"],
      tokens.syntax.string.fg,
    ),
    scope("variable.parameter", "", "italic"),
    scope(
      ["constant.numeric.decimal", "constant.numeric.hex"],
    ...
    // JSDOC
    scopeOrig(
      ["punctuation.definition.block.tag.jsdoc", "storage.type.class.jsdoc"],
      tokens.syntax.keyword.fg,
    ),
