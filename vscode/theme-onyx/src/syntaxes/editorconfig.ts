...
export default (tokens: Record<string, any>) => {
  return [
    ...
    scope(
      ["keyword.other.definition", "keyword.other.definition.root.editorconfig"],
      tokens.syntax.property.fg,
    ),
    scope(["constant.language.boolean"], tokens.syntax.keyword.fg),
    scope(
      ["punctuation.definition.brace.bracket", "punctuation.section.brace"],
    ...
    scope(
      ["constant.numeric.decimal.integer.int.editorconfig"],
